...
from src.embeddings import get_img_query_embeddings, get_text_query_embeddings, convert_vecs_to_text, convert_vecs_to_img
...
def retrieve_images(text_index, query_embedding, k):
    query_embedding_np = query_embedding.cpu().numpy().reshape(1, -1)
...
def retrive_landmarks_images(text_index, landmark_name_queries):
    ...
    retrieved_indices = retrieve_images(query_embeddings, k=1)
    retrieved_img_vectors = get_imgs_by_text_indices(text_index, retrieved_indices)
    retrieved_images = convert_vecs_to_img(retrieved_img_vectors)
...
def retrive_landmarks_names(img_index, img_query):
    ...
    retrieved_landmarks_names = convert_vecs_to_text(retrieved_text_vectors)
    return retrieved_landmarks_names
